...
def filter_electrical_transport(df: pd.DataFrame) -> pd.DataFrame:
    return df[
...
def invert_list(values: list[float]) -> list[float | None]:
    # 逆数を計算し、計算不能やオーバーフロー時のみ None にする。
    inverted = []
    for v in values:
        try:
            inv = 1.0 / v
            ...
            continue
        inverted.append(inv if math.isfinite(inv) else None)
    return inverted
...
def main():
    ...
    parser.add_argument("--out", default="sige_ElectricalConductivity_curves.csv")
    args = parser.parse_args()

    logging.basicConfig(level=logging.WARNING)

    # 入力 CSV を読み込み、Si-Ge と電気伝導率/抵抗率データに絞る。
    ...
    df_sige = filter_electrical_transport(df_sige)
    ...
    for row in df_sige.itertuples(index=True):
        ...
        # Electrical resistivity は逆数にして電気伝導率として扱う。
        if row.prop_y == "Electrical resistivity":
            y_list = invert_list(y_list)

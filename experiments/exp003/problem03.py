...
for epoch in range(num_epochs):
    ...
    huber_loss = torch.nn.functional.huber_loss(y_hat, y_train, delta=1.0, reduction="mean")
    MSE.backward()  # 勾配を計算
    ...
    if (epoch + 1) % 100 == 0:  # 100エポックごとに進捗を表示
        print(f"Epoch [{epoch + 1}/{num_epochs}], MSE: {MSE.item():.10f}, Huber: {huber_loss.item():.10f},  a: {a.item():.4f}, b: {b.item():.4f}, c: {c.item():.4f}")
...
with torch.no_grad():  # 勾配計算を無効にしてテストデータで予測
    ...
    test_huber = torch.nn.functional.huber_loss(y_test_hat, y_test, delta=1.0, reduction="mean")  # テストデータのHuber損失を計算
print(f"Final train MSE: {MSE.item():.10f}")  # 最終的な訓練データのMSEを表示
print(f"Final test  MSE: {test_mse.item():.10f}")  # 最終的なテストデータのMSEを表示
print(f"Final train Huber Loss: {huber_loss.item():.10f}")  # 最終的な訓練データのHuber損失を表示
print(f"Final test  Huber Loss: {test_huber.item():.10f}")  # 最終的なテストデータのHuber損失を表示
print(f"Learned params: a={a.item():.6f}, b={b.item():.6f}, c={c.item():.6f}")  # 学習されたパラメータを表示

# 誤差曲線（縦軸: 誤差, 横軸: エポック）を描画
plt.figure(figsize=(8, 5))
plt.plot(range(1, num_epochs + 1), mse_losses, label="Train MSE")
plt.plot(range(1, num_epochs + 1), huber_losses, label="Train Huber Loss")
plt.xlabel("Epoch")
plt.ylabel("Loss")
...
plt.legend()
plt.tight_layout()
plt.savefig("problem03_loss_curve.png", dpi=150)
plt.show()

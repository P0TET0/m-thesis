...
DEFAULT_OUTPUT_DIR = "data/output/starrydata2_step26_pred_obs_scatter_large_text"
...
def large_text_figure_name(figure_name):
    path = Path(figure_name)
    return f"{path.stem}_large_text{path.suffix}"
...
def draw_scatter(ax, data, spec, metric, title_prefix=""):
    ...
    ax.scatter(data["obs_value"], data["pred_value"], s=3, alpha=0.38, linewidths=0, color="#2563eb")
    ...
    version_label = {
        "direct_fitting": "Fitting",
        "ml_tau_prediction": "ML tau",
    }.get(spec["version"], spec["version"].replace("_", " "))
    title = f"{title_prefix}{spec['quantity']} {version_label}".strip()
    ax.set_title(title)
    ax.set_xlabel(f"Observed {spec['quantity']} ({spec['unit']})")
    ax.set_ylabel(f"Predicted {spec['quantity']} ({spec['unit']})")
...
def main():
        ...
        plot_rows.append(
            {
                ...
                "figure_file": str(Path("figures_step26") / large_text_figure_name(spec["figure"])),
    ...
    if matplotlib_ok:
        ...
        for spec in PLOT_SPECS + ZT_LOG_SPECS:
            ...
            if data.empty:
                figure_name = large_text_figure_name(spec["figure"])
                figures_skipped.append(f"{figure_name}: no valid rows; {note}")
                if spec["plot_id"] not in set(plot_list_df["plot_id"]):
                    plot_list_df.loc[len(plot_list_df)] = {
                        "plot_id": spec["plot_id"],
                        "quantity": spec["quantity"],
                        "version": spec["version"],
                        "figure_file": str(Path("figures_step26") / figure_name),
                        "axis_scale": spec["axis_scale"],
                        "n_rows": 0,
                        "n_plotted_rows": 0,
                        "status": "skipped",
            ...
            plt.close(fig)
            figures_created.append(str(Path("figures_step26") / figure_name))
        ...
        combined_figure_name = "figure_07_combined_pred_vs_obs_summary_large_text.png"
        combined_path = figures_dir / combined_figure_name
        if all((scatter_data["plot_id"] == spec["plot_id"]).any() for spec in PLOT_SPECS):
            ...
            plt.close(fig)
            figures_created.append(str(Path("figures_step26") / combined_figure_name))
            plot_list_df.loc[len(plot_list_df)] = {
                "plot_id": "combined_pred_vs_obs_summary",
                "quantity": "sigma/PF/ZT",
                "version": "direct_fitting_and_ml_tau_prediction",
                "figure_file": str(Path("figures_step26") / combined_figure_name),
                "axis_scale": "mixed",
                ...
                "status": "created",
                "note": "3 x 2 summary figure; sigma and PF log-log, ZT linear",
            }
            combined_created = "yes"
        else:
            figures_skipped.append(f"{combined_figure_name}: one or more main plots had no valid rows")
            plot_list_df.loc[len(plot_list_df)] = {
                "plot_id": "combined_pred_vs_obs_summary",
                "quantity": "sigma/PF/ZT",
                "version": "direct_fitting_and_ml_tau_prediction",

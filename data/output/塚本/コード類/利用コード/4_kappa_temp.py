...
def confirm_conductivity_coefficients(s, xi_g, T_range):
    results = {"T": [], "kappa_e": [], "kappa_L": [], "kappa": []}
    for T in T_range:
        sigma = calculate_conductivity(s, xi_F, xi_g)
        L = calculate_lorentz_number(s, xi_F, xi_g)
        κ_L = lattice_thermal_conductivity(T, Θ, v_s, kb, hbar, β, γ, M_kg, a)
    ...
    for i in range(len(results["T"])):
        print(f"T: {results['T'][i]:.3f} [K], kappa_e: {results['kappa_e'][i]:.3e} [W/mK], kappa_L: {results['kappa_L'][i]:.3e} [W/mK], kappa: {results['kappa'][i]:.3e} [W/mK]")

    # グラフの作成
    plt.figure(figsize=(10, 6))
    plt.plot(results["T"], [s * 1e6 for s in results["kappa_e"]], label="κ_e", linestyle='-')
    plt.plot(results["T"], [s * 1e6 for s in results["kappa_L"]], label="κ_L", linestyle='-.')
    plt.plot(results["T"], [s * 1e6 for s in results["kappa"]], label="κ", linestyle='--')

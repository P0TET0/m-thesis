...
def calculate_B(s):
    N_B = 2 * ((m * kb * T) / (2 * pi * hbar**2))**(3/2)
    mu = ((4 * q * tau_0) / (3 * mpmath.sqrt(pi) * m)) * gamma(s + 5/2)
    kappa_L = mpmath.mpf(1)  # 定数として仮定
    B = ((kb **2) / q) * N_B * ((T * mu) / kappa_L)  # Bをここで計算
...
for xi_F in xi_F_range:
    B_32 = calculate_B(3/2)
    B_m12 = calculate_B(-1/2)

    sigma_32, alpha_32, L_32, ZT_32 = calculate_properties(3/2, xi_F, B_32)
    sigma_m12, alpha_m12, L_m12, ZT_m12 = calculate_properties(-1/2, xi_F, B_m12)
...
# ξ_F の値に対して各 ZT 値を格納
for xi_F, ZT_32, ZT_m12, ZT_var2_32, ZT_var2_m12 in zip(xi_F_range, ZT_values_32, ZT_values_m12, ZT_var2_values_32, ZT_var2_values_m12):
    xi_F_values.append(xi_F)
    ZT_results_32.append(ZT_32)
    ZT_results_m12.append(ZT_m12)
    ZT_var2_results_32.append(ZT_var2_32)
    ZT_var2_results_m12.append(ZT_var2_m12)

# DataFrame に変換
results_df = pd.DataFrame({
    'xi_F': xi_F_values,
    'ZT (s=3/2)': ZT_results_32,
    'ZT (s=-1/2)': ZT_results_m12,
    'ZT_var2 (s=3/2)': ZT_var2_results_32,
    'ZT_var2 (s=-1/2)': ZT_var2_results_m12
})

...
# 温度範囲の定義を冒頭で行う
T_range = [mpmath.mpf(val) for val in np.linspace(10, 1300, 13)]  # 温度範囲 (300K～1300K)
...
# 1/τ_ep の定義
def tau_ep_inv(xi, xi_F, E, m_star, v_s, d, phi):
...
# 1/τ_pd の定義
def tau_pd_inv(xi, v_s, T, y, delta_M, M_g, ε_S, delta_a, a):
    term1 = (1 / (4 * np.pi)) * ((a / v_s)**3) * ((kb * T / hbar)**4) * y * (1 - y)
    term2 = ((delta_M / M_g) ** 2 + ε_S * (delta_a / a) ** 2)
    return term1 * term2 * xi ** 4

# 1/τ_gb の定義
def tau_gb_inv(v_s, L):
    return v_s / L

# 1/τ_C の計算
def tau_C_inv(xi, xi_F, T, Θ, β, γ, E, y, L,  m_star, v_s, d, phi, delta_M, M_kg, ε_S, delta_a, a):
    # τ_N と τ_U と τ_ep と τ_pd と τ_gb を考慮
    τ_N_inv_val = tau_N_inv(xi, T, Θ, β, γ, M_kg, a)
    τ_U_inv_val = tau_U_inv(τ_N_inv_val, β)
    τ_ep_inv_val = tau_ep_inv(xi, xi_F, E, m_star, v_s, d, phi)
    #τ_pd_inv_val = tau_pd_inv(xi, v_s, T, y, delta_M, M_kg, ε_S, delta_a, a)
    #τ_gb_inv_val = tau_gb_inv(v_s, L)
    #return τ_N_inv_val + τ_U_inv_val + τ_ep_inv_val + τ_pd_inv_val + τ_gb_inv_val
    return τ_N_inv_val + τ_U_inv_val + τ_ep_inv_val

# τ_C を積分中で動的に計算
def tau_C(xi, xi_F, T, Θ, β, γ, E, y, L, m_star, v_s, d, phi, delta_M, M_kg, ε_S, delta_a, a):
    τ_C_val = 1 / tau_C_inv(xi, xi_F, T, Θ, β, γ, E, y, L, m_star, v_s, d, phi, delta_M, M_kg, ε_S, delta_a, a)
    return τ_C_val

# I1の積分
def I1_integrand(xi, T, Θ, β, γ, E, y, L, m_star, v_s, d, phi, delta_M, M_kg, ε_S, delta_a, a):
    τ_C_val = tau_C(xi, xi_F, T, Θ, β, γ, E, y, L, m_star, v_s, d, phi, delta_M, M_kg, ε_S, delta_a, a)  # xi に依存する τ_C の計算
    return τ_C_val * (xi ** 4 * np.exp(xi)) / ((np.exp(xi) - 1) ** 2)

def I1(T, Θ, β, γ, E, y, L, m_star, v_s, d, phi, delta_M, M_kg, ε_S, delta_a, a):
    return quad(I1_integrand, 0, Θ / T, args=(T, Θ, β, γ, E, y, L, m_star, v_s, d, phi, delta_M, M_kg, ε_S, delta_a, a))[0]

# I2の積分
def I2_integrand(xi, xi_F, T, Θ, β, γ, E, y, L, m_star, v_s, d, phi, delta_M, M_kg, ε_S, delta_a, a):
    τ_C_val = tau_C(xi, xi_F, T, Θ, β, γ, E, y, L, m_star, v_s, d, phi, delta_M, M_kg, ε_S, delta_a, a)  # xi に依存する τ_C の計算
    τ_N_val = 1 / tau_N_inv(xi, T, Θ, β, γ, M_kg, a)  # xi に依存する τ_N の計算
    return (τ_C_val / τ_N_val) * (xi ** 4 * np.exp(xi)) / ((np.exp(xi) - 1) ** 2)

def I2(xi_F, T, Θ, β, γ, E, y, L,  m_star, v_s, d, phi, delta_M, M_kg, ε_S, delta_a, a):
    return quad(I2_integrand, 0, Θ / T, args=(xi_F, T, Θ, β, γ, E, y, L,  m_star, v_s, d, phi, delta_M, M_kg, ε_S, delta_a, a))[0]

# I3の積分
def I3_integrand(xi, xi_F, T, Θ, β, γ, E, y, L, m_star, v_s, d, phi, delta_M, M_kg, ε_S, delta_a, a):
    τ_C_val = tau_C(xi, xi_F, T, Θ, β, γ, E, y, L, m_star, v_s, d, phi, delta_M, M_kg, ε_S, delta_a, a)  # xi に依存する τ_C の計算
    τ_N_val = 1 / tau_N_inv(xi, T, Θ, β, γ, M_kg, a)  # xi に依存する τ_N の計算
    return (1 / τ_N_val) * (1 - (τ_C_val / τ_N_val)) * (xi ** 4 * np.exp(xi)) / ((np.exp(xi) - 1) ** 2)

def I3(xi_F, T, Θ, β, γ, E, y, L,  m_star, v_s, d, phi, delta_M, M_kg, ε_S, delta_a, a):
    return quad(I3_integrand, 0, Θ / T, args=(xi_F, T, Θ, β, γ, E, y, L, m_star, v_s, d, phi, delta_M, M_kg, ε_S, delta_a, a))[0]

# 格子熱伝導率 κ_L の計算
def lattice_thermal_conductivity(xi_F, T, Θ, β, γ, E, y, L, m_star, v_s, d, phi, delta_M, M_kg, ε_S, delta_a, a):
    I1_val = I1(T, Θ, β, γ, E, y, L, m_star, v_s, d, phi, delta_M, M_kg, ε_S, delta_a, a)
    I2_val = I2(xi_F, T, Θ, β, γ, E, y, L,  m_star, v_s, d, phi, delta_M, M_kg, ε_S, delta_a, a)
    I3_val = I3(xi_F, T, Θ, β, γ, E, y, L,  m_star, v_s, d, phi, delta_M, M_kg, ε_S, delta_a, a)
...
def confirm_conductivity_coefficients(s, xi_g, T_range):
    ...
    for T, xi_F in zip(T_range, xi_F_results):  # 温度と対応する xi_F を利用
        ...
        κ_L = lattice_thermal_conductivity(xi_F, T, Θ, β, γ, E, y, L,  m_star, v_s, d, phi, delta_M, M_kg, ε_S, delta_a, a)
...
T_range = [mpmath.mpf(val) for val in np.linspace(10, 1300, 13)]
s = -1/2  # 状態密度の有効質量の次元数

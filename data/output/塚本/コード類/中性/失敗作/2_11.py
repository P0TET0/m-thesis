...
# 温度範囲の設定
T_range = np.linspace(200, 1300, 100)  # 200K から 1300K の範囲を100分割

# ξ_Fの値のリスト
xi_F_values = [0.8726856, -0.3657572, 0.2712006, 1.4489438, 2.0160967]

# 計算結果を格納する辞書
results = {
    "alpha": {xi_F: [] for xi_F in xi_F_values},
    "sigma": {xi_F: [] for xi_F in xi_F_values},
    "kappa": {xi_F: [] for xi_F in xi_F_values},
    "ZT": {xi_F: [] for xi_F in xi_F_values},
}
...
# 各温度に対して各ξ_Fの値で計算
for xi_F in xi_F_values:
    for T in T_range:
        # 計算
        alpha = calculate_alpha(3/2, xi_F, xi_g)  # αの計算
        sigma = calculate_conductivity(3/2, xi_F, xi_g)  # σの計算
        L = calculate_lorentz_number(3/2, xi_F, xi_g)  # ローレンツ数の計算
        kappa_e = L * T * sigma  # 電子熱伝導率の計算
        kappa_L = lattice_thermal_conductivity(T, Θ, v_s, kb, hbar, β, γ, M_kg, a)  # 格子熱伝導率の計算（ダミー）
        kappa = kappa_e + kappa_L  # 総熱伝導率
        ZT = calculate_ZT(3/2, xi_F, xi_g)  # ZTの計算

        # 結果の保存
        results["alpha"][xi_F].append(alpha * 1000)  # [mV/K]に変換
        results["sigma"][xi_F].append(sigma)  # [S/m]
        results["kappa"][xi_F].append(kappa)  # [W/(m*K)]
        results["ZT"][xi_F].append(ZT)

# ξ_Fと対応するラベルの辞書
xi_F_labels = {
    -0.3657572: "N_A=2.15e25",
    0.2712006: "N_D=4.64e25",
    0.8726856: "N_D=1e26",
    1.4489438: "N_D=2.15e26",
    2.0160967: "N_D=4.64e26"
}
...
# αのプロット（線形スケール）
for xi_F, label in xi_F_labels.items():
    axes[0, 0].plot(T_range, results["alpha"][xi_F], label=label)
axes[0, 0].set_xlabel("T [K]")
axes[0, 0].set_ylabel("α [mV/K]")
axes[0, 0].legend()

# σのプロット
for xi_F, label in xi_F_labels.items():
    axes[0, 1].plot(T_range, results["sigma"][xi_F], label=label)
axes[0, 1].set_xlabel("T [K]")
axes[0, 1].set_ylabel("σ [S/m]")
axes[0, 1].legend()

# κのプロット
for xi_F, label in xi_F_labels.items():
    axes[1, 0].plot(T_range, results["kappa"][xi_F], label=label)
axes[1, 0].set_xlabel("T [K]")
axes[1, 0].set_ylabel("κ [W/(m*K)]")
axes[1, 0].legend()

# ZTのプロット
for xi_F, label in xi_F_labels.items():
    axes[1, 1].plot(T_range, results["ZT"][xi_F], label=label)
axes[1, 1].set_xlabel("T [K]")
axes[1, 1].set_ylabel("ZT")
axes[1, 1].legend()

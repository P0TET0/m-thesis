...
def find_fermi_level(start, end, step, tolerance):
    ...
    for initial_guess in mpmath.arange(start, end + step, step):
        try:
            xi_F_solution = mpmath.findroot(neutrality_condition, initial_guess, solver='secant', tol=mpmath.mpf(tolerance), maxsteps=100)
            value = neutrality_condition(xi_F_solution)
            formatted_value = mpmath.nstr(value, 10)
            
            # |n - (N_D - n_D) - p| が tol より小さいか確認
            if mpmath.fabs(value) < mpmath.mpf(tolerance):
                results.append((float(initial_guess), formatted_value))
            else:
                results.append((float(initial_guess), "Not converged"))
        except Exception as e:
            # エラーメッセージから数値を抽出
            match = re.search(r'(\d+\.\d+e[+\-]\d+)', str(e))
            formatted_value = match.group(1) if match else "Error"
        results.append((float(initial_guess), formatted_value))
        initial_guesses.append(float(initial_guess))
        values.append(float(formatted_value) if formatted_value != "Error" else None)

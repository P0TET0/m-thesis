...
def calculate_kappa(s, xi_F, xi_g):
    sigma = calculate_conductivity(s, xi_F, xi_g)
...
def confirm_conductivity_coefficients(s, xi_g, xi_F_range):
    results = {"xi_F": [], "kappa_e": [], "kappa_L": [], "kappa": []}
    for xi_F in xi_F_range:
        ...
        results["kappa_e"].append(float(kappa_e))   # mpf を float に変換
        results["kappa_L"].append(float(kappa_L))   # mpf を float に変換
        results["kappa"].append(float(kappa))  # mpf を float に変換

    # 数値リストの出力
    for i in range(len(results["xi_F"])):
        print(f"xi_F: {results['xi_F'][i]:.3f}, kappa_e: {results['kappa_e'][i]:.3e} [V/K], kappa_L: {results['kappa_L'][i]:.3e} [V/K], kappa: {results['kappa'][i]:.3e} [V/K]")

    # グラフの作成
    plt.figure(figsize=(10, 6))
    plt.plot(results["xi_F"], [s for s in results["kappa_e"]], label="κ_e", linestyle='-')
    plt.plot(results["xi_F"], [s for s in results["kappa_L"]], label="κ_L", linestyle='-.')
    plt.plot(results["xi_F"], [s for s in results["kappa"]], label="κ", linestyle='--')
